...
::            for PHP 4.3 C:\phpdev\php-4.3-Win32\cli\php.exe
  ...
  SET phpCli=\PHP\xampp\php\.\php.exe
...
IF '%1'=='' (
  ...
  "%phpCli%" -d include_path="\PHP\xampp\php\pear" "\xampp\php\phpdoc" %*
  GOTO :EOF
)
...
:run
SET found=1
ECHO Starting: "%phpCli%" "\PHP\xampp\php\phpdoc" -c "%iniFile%"
ECHO.
"%phpCli%"  -d include_path="\PHP\xampp\php\pear" "\xampp\php\phpdoc" -c "%iniFile%"
GOTO :EOF
